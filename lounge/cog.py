...
class LoungeCog(commands.Cog, name='Lounge'):
    ...
    @commands.command(
        name='penalty',
        brief='Shows penalty information'
    )
    async def penalty(self, ctx: commands.Context, *, player: Optional[str] = None):
        # สร้างข้อมูลเพื่อทดสอบ
        penalty_data = {
            'id': 1,
            'season': 10,
            'awarded_on': (2023, 10, 1),
            'is_strike': True,
            'prev_mmr': 1500,
            'new_mmr': 1450,
            'amount': 50,
        ...
        # สร้างอินสแตนซ์ของคลาส Penalty
        sample_penalty = Penalty.loads(penalty_data)

        if player is not None:
            # ใช้ player ที่คุณระบุ
            # คุณสามารถนำข้อมูลจาก Discord มาหาข้อมูลของผู้เล่นจริงๆ
            # และใช้ฟังก์ชันเพื่อดึงข้อมูลคะแนน (strikes) ของผู้เล่นจริงๆ
            player_data = {
                'id': 2,
                'season': 10,
                'awarded_on': datetime(2023, 10, 5),
                'is_strike': True,
                'prev_mmr': 1450,
                'new_mmr': 1400,
                'amount': 50,
            ...
            # สร้างอินสแตนซ์ของคลาส Penalty สำหรับผู้เล่นที่คุณระบุ
            player_penalty = Penalty.loads(player_data)

            # ส่งข้อมูลของผู้เล่นที่คุณระบุไปยังผู้ใช้
            await ctx.send(f'Penalty information for {player_penalty.player_name}:')
            await ctx.send(f'ID: {player_penalty.id}')
            await ctx.send(f'Season: {player_penalty.season}')
            await ctx.send(f'Awarded On: {player_penalty.awarded_on}')
            await ctx.send(f'Is Strike: {player_penalty.is_strike}')
            await ctx.send(f'Previous MMR: {player_penalty.prev_mmr}')
            await ctx.send(f'New MMR: {player_penalty.new_mmr}')
            await ctx.send(f'Amount: {player_penalty.amount}')
            await ctx.send(f'Deleted On: {player_penalty.deleted_on}')
            await ctx.send(f'Player ID: {player_penalty.player_id}')
        else:
            # ส่งข้อมูลของตัวอย่าง Penalty ที่คุณสร้างขึ้นเพื่อทดสอบ
            await ctx.send(f'Penalty information for SamplePlayer:')
            await ctx.send(f'ID: {sample_penalty.id}')
            await ctx.send(f'Season: {sample_penalty.season}')
            await ctx.send(f'Awarded On: {sample_penalty.awarded_on}')
            await ctx.send(f'Is Strike: {sample_penalty.is_strike}')
            await ctx.send(f'Previous MMR: {sample_penalty.prev_mmr}')
            await ctx.send(f'New MMR: {sample_penalty.new_mmr}')
            await ctx.send(f'Amount: {sample_penalty.amount}')
